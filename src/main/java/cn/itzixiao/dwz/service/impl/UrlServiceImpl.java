package cn.itzixiao.dwz.service.impl;
...
import cn.itzixiao.dwz.entity.UrlMap;
import cn.itzixiao.dwz.mapper.UrlMapper;
...
public class UrlServiceImpl implements UrlService {
	...
	//最近使用的短链接缓存过期时间(分钟)
	private static final long TIMEOUT = 7200;
	private static final String URL_CACHE_KEY_PREFIX = "short:url:";
	//创建布隆过滤器
	private static final BitMapBloomFilter FILTER = BloomFilterUtil.createBitMap(10);

	@Override
	public String getLongUrlByShortUrl(String shortURL) {
		String cacheKey = URL_CACHE_KEY_PREFIX + shortURL;
		//查找Redis中是否有缓存
		...
		if (longURL != null) {
			//数据库有此短链接，添加缓存
			redisTemplate.opsForValue().set(cacheKey, longURL, TIMEOUT, TimeUnit.MINUTES);
		}
		return longURL;
	}
	...
	public String saveUrlMap(String shortURL, String longURL, String originalURL) {
		...
		//在布隆过滤器中查找是否存在
			...
			//存在，从Redis中查找是否有缓存
			String redisLongURL = redisTemplate.opsForValue().get(cacheKey);
			...
			try {
				...
				FILTER.add(shortURL);
				//添加缓存
				redisTemplate.opsForValue().set(cacheKey, originalURL, TIMEOUT, TimeUnit.MINUTES);
			} catch (Exception e) {
